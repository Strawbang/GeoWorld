    ...
    public ArrayList<Pays> getCountryList (){
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++)
            {
                ...
                //Log.i("tag7", drapeau);
                String superficie = jsonObj.getString("area");
                String continent = jsonObj.getString("region");
                //Log.i("tag8", superficie);
                Pays currentPays = new Pays(nomPays, capital, drapeau, superficie, continent);
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++)
            {
                ...
                if (jsonObj.getString("region").equals(filter)){
                    ...
                    String superficie = jsonObj.getString("area");
                    String continent = jsonObj.getString("region");
                    currentPays = new Pays(nomPays, capital, drapeau, superficie, continent);
                    // Ajout dans l'ArrayList
                    items.add(currentPays);
                }
            }
